...
복붙해서 바로 쓰는 완성형 예시입니다. 모두 `init()` 형태이며, React는 같은 객체를 `QuipierProvider config`/`QuipierComments` props로 넘기면 됩니다.
...
기본 이니셜 아바타 대신 실제 프로필 이미지를 — 나머지 row는 그대로(부분 슬롯).
...
init({
  ...
  slots: {
    avatar: (view) => {
      ...
      img.src = `https://cdn.example.com/u/${view.author.id}.png`;
...
### 6) 기본 디자인 + 배지만 덧붙이기 (`defaultNode` 래핑)

기본 row를 그대로 쓰되 내 댓글에만 "내 댓글" 배지를 추가.
...
init({
  ...
  slots: {
    comment: (view, ctx) => {
      if (!view.author.isOwn) return null; // 남의 댓글은 기본 그대로
      const wrap = document.createElement("div");
      const tag = document.createElement("span");
      tag.textContent = "내 댓글";
      tag.style.cssText =
        "display:inline-block;font-size:11px;font-weight:700;color:#16a34a;background:#dcfce7;border-radius:999px;padding:2px 8px;margin-bottom:6px";
      wrap.append(tag, ctx.defaultNode()); // 기본 렌더 + 배지
      return wrap;
    },
  },
});
